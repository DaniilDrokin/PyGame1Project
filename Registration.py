...
def registration():  # Функция, отвечающая за регистрацию пользователя
    ...
    class Line(pygame.sprite.Sprite):  # Класс поля, где отображается логин и пароль
        ...
        def return_name(self):  # Возвращает имя строки(login/password)
            return self.name
    ...
    for i in range(2):  # Создание полей логина и пароля
        y = coordinates[i]
        name = lines[i]
        Line(y, name)
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:  # Запись в БД пользователя, если оба поля заполнены и нажат пробел
                if event.key == pygame.K_SPACE and password_text != '' and login_text != '':
                    ...
                    if was_login_before:  # Добавление в файл txt информации о ранее зарегистрированном
                    ...
                    conn.commit()
                    conn.close()
            if event.type == pygame.MOUSEBUTTONDOWN:  # Определение на какое поле кликнул пользователь
            ...
            if event.type == pygame.KEYDOWN and login_flag:
                if event.type == pygame.KEYDOWN and event.key == pygame.K_BACKSPACE and login_text != '':
                    ...
                    if 65 <= event.key <= 122 and event.key != 91 and event.key != 92 and event.key != 93\
                            and len(login_text) < 12:  # Проверка на маленькую английскую букву
            ...
            # Добавление текста в поле пароля
            if event.type == pygame.KEYDOWN and password_flag:
                if event.type == pygame.KEYDOWN and event.key == pygame.K_BACKSPACE and password_text != '':
                    # Проверка на маленькую английскую букву введенного пароля
                    password_text = password_text[:-1]
                    text_4 = font.render(password_text, True, (0, 0, 0))
                else:
                    if 65 <= event.key <= 122 and event.key != 91 and event.key != 92 and event.key != 93\
                            and len(password_text) < 12:  # Проверка на маленькую английскую букву
        ...
        if error_flag and not space_flag:  # Вывод ошибки ввода значения(при этом ошибка на пробел не выводится,
            # потому что именно по нажатию проела мы переходим на следующий экран
            screen.blit(text_5, (75, 230))
            screen.blit(text_6, (35, 260))
            screen.blit(text_8, (0, 290))
        if error_log_pas:  # Вывод ошибки логина и пароля
